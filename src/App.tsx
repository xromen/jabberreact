...
export default function App() {
  ...
  useEffect(() => {
    ...
    const onLastSeen = (event: Event) => {
      const detail = (event as CustomEvent<Pick<Contact, "jid" | "lastSeen">>)
        .detail;
      setContacts((current) =>
        current.map((contact) =>
  ...
  async function loadOlderMessages() {
    ...
    setLoadingOlderMessages(true);
    try {
      const page = await getMessagePage(activeAccount, conversation, before);
      if (
  ...
  function showNotification(from: string, body: string) {
    ...
    const notification = new Notification(from.split("@")[0], {
      ...
      icon: "/favicon.svg",
      tag: from,
      requireInteraction: true, // Нотификация не будет автоматически закрываться
      silent: false,
    });
  ...
  return (
    <main className="app-shell">
      <DynamicFavicon
        count={Object.values(unread).reduce((acc, cur) => acc + cur, 0)}
      />
      <ServerRail
        groups={groups}
